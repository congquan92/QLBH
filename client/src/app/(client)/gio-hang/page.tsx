...
import {
  ...
  isCartItemAvailable,
} from "@/app/(client)/gio-hang/_components/cart-utils";
...
export default function CartPage() {
  ...
  const [paymentType, setPaymentType] = useState<"COD" | "BANK_TRANSFER">(
    "COD",
  );
  ...
  const isLoading =
    isAuthLoading || (isAuthenticated && lastLoadedToken !== sessionToken);
  ...
  useEffect(() => {
    setSelectedItemIds((current) =>
      current.filter((id) =>
        cartItems.some((item) => item.id === id && isCartItemAvailable(item)),
      ),
    );
  }, [cartItems]);
  ...
  const {
    ...
    isLoadingDistricts,
    isLoadingWards,
  } = useGhnAddressOptions(newAddress.province_id, newAddress.district_id);
  ...
  const resolveSelectedAddress = () => {
    ...
    if (
      ...
      throw new Error(
        "Địa chỉ đã chọn đang thiếu dữ liệu, vui lòng cập nhật trong trang tài khoản hoặc nhập địa chỉ mới.",
      );
    }
  ...
  const handleCheckout = async () => {
    ...
    const orderItems = getOrderItemsFromCart(
      selectedItems.filter((item) => isCartItemAvailable(item)),
    );
    if (orderItems.length !== selectedItems.length) {
      toast.error(
        "Có sản phẩm đã chọn thiếu biến thể hoặc không còn khả dụng. Vui lòng kiểm tra lại sản phẩm.",
      );
      return;
    }
    ...
    try {
      ...
      if (!createdOrderId) {
        ...
        const latestList =
          (latestOrderResponse.data as { data?: Array<{ id?: number }> })
            ?.data ?? [];
      ...
      if (paymentType === "BANK_TRANSFER") {
        ...
        const paymentUrl =
          typeof paymentData === "string"
            ? paymentData
            : typeof paymentData?.paymentUrl === "string"
              ? paymentData.paymentUrl
              : "";

        // Chỉ xóa giỏ sau khi callback VNPay xác nhận thành công.
        sessionStorage.setItem(
          `pending_vnpay_cart_${createdOrderId}`,
          JSON.stringify(selectedIdsSnapshot),
        );
        ...
        toast.success(
          "Đơn hàng đã tạo. Không lấy được liên kết thanh toán online, vui lòng kiểm tra trong lịch sử đơn hàng.",
        );
      }
      ...
      setCartItems((current) =>
        current.filter((item) => !selectedIdsSnapshot.includes(item.id)),
      );
      setSelectedItemIds([]);
      setNote("");
      ...
      <div className="mx-auto max-w-4xl px-4 py-16">
        <div className="border border-dashed border-gray-300 bg-gray-50 p-10 text-center">
          ...
          <p className="mt-3 text-sm leading-6 text-gray-600">
            Bạn cần đăng nhập để xem và quản lý giỏ hàng của mình. Đừng lo, chỉ
            mất vài giây!
          </p>
          <div className="mt-6 flex justify-center gap-3">
            <Link
              href="/dang-nhap?redirect=%2Fgio-hang"
              className="bg-red-600 px-5 py-3 text-sm font-semibold text-white transition-colors hover:bg-red-700"
            >
              Đăng nhập để xem giỏ hàng
            </Link>
            <Link
              href="/san-pham"
              className="border border-gray-300 px-5 py-3 text-sm font-semibold text-gray-700 transition-colors hover:border-gray-900 hover:text-gray-900"
            >
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-10">
      ...
      {cartItems.length === 0 ? (
        ...
        <div className="grid gap-8 lg:grid-cols-[1.5fr_0.8fr]">
          ...
          <CartCheckoutSummary
            ...
            onNewAddressChange={(updater) =>
              setNewAddress((current) => updater(current))
            }
      ...
      {invoice ? (
        <CartInvoice
          invoice={invoice}
          fallbackTotalAmount={lastCheckoutAmount || totalAmount}
        />
      ) : null}
    </div>
  );
