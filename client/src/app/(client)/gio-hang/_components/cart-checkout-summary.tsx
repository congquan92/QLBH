...
interface CartCheckoutSummaryProps {
    ...
    onNewAddressChange: (updater: (current: NewAddressForm) => NewAddressForm) => void;
...
function VoucherBadge({ voucher }: { voucher: Voucher }) {
    const discount = voucher.type === "PERCENTAGE" ? `Giảm ${voucher.discountValue}%` : `Giảm ${Helper.formatPrice(String(voucher.discountValue ?? 0))}`;

    const extra = voucher.maxDiscountValue != null ? ` (tối đa ${Helper.formatPrice(String(voucher.maxDiscountValue))})` : "";

    const minOrder = voucher.minDiscountValue != null && Number(voucher.minDiscountValue) > 0 ? ` · Đơn từ ${Helper.formatPrice(String(voucher.minDiscountValue))}` : "";
    ...
    return (
        <span className="text-xs text-gray-600">
            {voucher.isShipping ? <Truck className="mr-1 inline-block size-3 text-blue-500" /> : <Tag className="mr-1 inline-block size-3 text-red-500" />}
            {discount}
            {extra}
            {minOrder}
            {usageText}
        ...
        <aside className="h-fit border border-gray-200 bg-gray-50 p-6 space-y-6">
            ...
            <div className="space-y-3 text-sm text-gray-600">
                ...
                {voucherDiscountAmount > 0 && (
                    <div className="flex items-center justify-between text-green-700">
                        ...
                        <span className="font-medium">− {Helper.formatPrice(String(voucherDiscountAmount))}</span>
                    </div>
                )}
            </div>
            ...
            <section className="space-y-3 border-t border-gray-200 pt-4">
                ...
                <div className="flex gap-2">
                    <button type="button" onClick={() => onUseNewAddressChange(false)} className={`border px-3 py-2 text-xs font-semibold ${!useNewAddress ? "border-red-600 bg-red-600 text-white" : "border-gray-300 bg-white text-gray-700"}`}>
                        Chọn từ tài khoản
                    </button>
                    <button type="button" onClick={() => onUseNewAddressChange(true)} className={`border px-3 py-2 text-xs font-semibold ${useNewAddress ? "border-red-600 bg-red-600 text-white" : "border-gray-300 bg-white text-gray-700"}`}>
                        Nhập địa chỉ mới
                    </button>
                </div>
                    ...
                    <div className="grid gap-2 text-sm">
                        ...
                        <select
                            ...
                            onChange={(event) => {
                                ...
                                const selectedProvince = provinceOptions.find((item) => String(item.ProvinceID) === nextProvinceId);
                                onNewAddressChange((current) => ({
                                    ...current,
                        ...
                        >
                            <option value="">{isLoadingProvinces ? "Đang tải tỉnh/thành..." : "Chọn Tỉnh/Thành"}</option>
                        ...
                        <select
                            ...
                            onChange={(event) => {
                                ...
                                const selectedDistrict = districtOptions.find((item) => String(item.DistrictID) === nextDistrictId);
                                onNewAddressChange((current) => ({
                                    ...current,
                        ...
                        <select
                            ...
                            onChange={(event) => {
                                ...
                                const selectedWard = wardOptions.find((item) => item.WardCode === nextWardId);
                                onNewAddressChange((current) => ({
                                    ...current,
                                    ward_id: nextWardId,
                            ...
                            className="h-9 border border-gray-300 px-3 disabled:bg-gray-100"
                        >
                            <option value="">{isLoadingWards ? "Đang tải phường/xã..." : "Chọn Phường/Xã"}</option>
            ...
            <section className="space-y-3 border-t border-gray-200 pt-4">
                ...
                {selectedVoucher ? (
                    <div className="flex items-center justify-between border border-green-500 bg-green-50 px-3 py-2 text-sm">
                        <div className="flex flex-col gap-0.5">
                            <span className="font-semibold text-green-800">{selectedVoucher.description || `Voucher #${selectedVoucher.id}`}</span>
                            <VoucherBadge voucher={selectedVoucher} />
                        </div>
                        <button type="button" onClick={() => onSelectVoucher(null)} className="ml-2 text-gray-400 hover:text-red-500" title="Bỏ chọn voucher">
                            <X className="size-4" />
                        </button>
                    </div>
                ) : null}
                ...
                {vouchers.length > 0 ? (
                    ...
                    >
                        ...
                        {showVoucherList ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}
                    </button>
                ) : (
                    <p className="text-xs text-gray-400 italic">Bạn không có voucher khả dụng cho đơn này.</p>
                )}
                ...
                {showVoucherList && vouchers.length > 0 && (
                    <div className="max-h-64 space-y-2 overflow-y-auto border border-gray-200 bg-white p-2">
                        {vouchers.map((voucher) => {
                            ...
                            return (
                                <label key={voucher.id} className={`flex cursor-pointer items-start gap-3 border p-3 text-sm transition-colors ${isSelected ? "border-red-500 bg-red-50" : "border-gray-200 hover:border-gray-300 hover:bg-gray-50"}`}>
                                    <input
                                        type="radio"
                                        name="voucher"
                                    ...
                                    <div className="flex flex-col gap-1 min-w-0">
                                        <span className="font-semibold text-gray-900 truncate">{voucher.description || `Voucher #${voucher.id}`}</span>
                                        <VoucherBadge voucher={voucher} />
                                        {voucher.endDate && <span className="text-[11px] text-gray-400">HSD: {new Date(voucher.endDate).toLocaleDateString("vi-VN")}</span>}
                                    </div>
                                </label>
                            );
                        })}
            ...
            <section className="space-y-3 border-t border-gray-200 pt-4">
                ...
                <label className={`flex cursor-pointer items-center gap-2 border p-3 text-sm ${paymentType === "COD" ? "border-red-600 bg-red-50" : "border-gray-200 bg-white"}`}>
                    <input type="radio" name="payment-type" checked={paymentType === "COD"} onChange={() => onPaymentTypeChange("COD")} />
                    Tiền mặt (COD)
                </label>
                <label className={`flex cursor-pointer items-center gap-2 border p-3 text-sm ${paymentType === "BANK_TRANSFER" ? "border-red-600 bg-red-50" : "border-gray-200 bg-white"}`}>
                    <input type="radio" name="payment-type" checked={paymentType === "BANK_TRANSFER"} onChange={() => onPaymentTypeChange("BANK_TRANSFER")} />
                    Trực tuyến (VNPay)
                </label>
