...
interface AddressesSectionProps {
  ...
  onFormChange: (
    updater: (current: AddressFormState) => AddressFormState,
  ) => void;
  onAddAddress: () => void;
            ...
            addresses.map((address) => (
              <article key={address.id} className="border border-gray-200 p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  ...
                  address.isDefault ? (
                    <span className="border border-red-200 bg-red-50 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-red-600">
                      Mặc định
                    </span>
                  ) : null}
                </div>
