...
export function CategoryTree({
    ...
    function onChildDrop(
        e: React.DragEvent,
        dropParentIdx: number,
        dropChildIdx: number
    ) {
        e.preventDefault();
        e.stopPropagation();
    ...
    const totalCount = localCategories.reduce(
        (acc, cat) => acc + 1 + (cat.childCategory?.length ?? 0),
        0
    );
        ...
        <div className="space-y-3">
            ...
            <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground">
                    <span className="font-medium text-foreground">{totalCount}</span> danh mục
                    ·{" "}
                ...
                {isDirty && (
                    <div className="flex items-center gap-2">
                        <span className="text-xs text-amber-600 font-medium">
                            ● Có danh mục con đổi nhóm chưa lưu
                        </span>
                        <Button
                            variant="outline"
                            size="sm"
                            ...
                            <div className="flex items-center gap-2 px-3 py-2.5">
                                ...
                                >
                                    {hasChildren ? (
                                        ...
                                        <FolderTree className="h-4 w-4 opacity-25" />
                                    )}
                                </button>
                                    ...
                                    {category.childCategory.map((child, childIdx) => {
                                        ...
                                        return (
                                            <div
                                                ...
                                                draggable
                                                onDragStart={(e) =>
                                                    onChildDragStart(e, parentIdx, childIdx)
                                                }
                                                onDragOver={(e) =>
                                                    onChildDragOver(e, parentIdx, childIdx)
                                                }
                                                onDrop={(e) =>
                                                    onChildDrop(e, parentIdx, childIdx)
                                                }
                                                onDragEnd={onChildDragEnd}
                                                className={`flex items-center gap-2 rounded-md border bg-background px-3 py-2 transition-all ${
                                                    isDraggingChild
                                            ...
                                            >
                                                ...
                                                <Badge
                                                    variant={
                                                        childActive ? "outline" : "destructive"
                                                    }
                                                    className="shrink-0 text-xs"
                                                >
