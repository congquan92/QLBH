...
import { Boxes, Check, ImagePlus, Layers3, Loader2, PackageCheck, Plus, Save, Trash2, Truck } from "lucide-react";
                    ...
                    <div className="space-y-4 px-5 pb-24 pt-3">
                        <section className="rounded-lg border bg-card p-4">
                            ...
                            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                                ...
                                <div className="space-y-2 md:col-span-2 xl:col-span-2">
                                    ...
                                    <Input id="product-video" value={form.video} onChange={(event) => onChange("video", event.target.value)} placeholder="https://res.cloudinary.com/.../video.mp4" />
                                </div>
                            </div>
                        ...
                        <section className="space-y-3 rounded-lg border bg-card p-4">
                            ...
                            {form.attributes.length === 0 ? (
                                <div className="rounded-xl border border-dashed p-4 text-sm text-muted-foreground">Chưa có thuộc tính. Ví dụ: Màu sắc, Kích thước.</div>
                            ) : (
                                <div className="space-y-3">
                                    {form.attributes.map((attribute, index) => (
                                        <div key={attribute.key} className="space-y-3 rounded-md border bg-background p-3">
                                            <div className="grid gap-2 md:grid-cols-[180px_1fr_auto] md:items-end">
                                                <div className="space-y-1">
                                                    <Label className="text-xs text-muted-foreground">Thuộc tính {index + 1}</Label>
                                                    <Input value={attribute.name} onChange={(event) => updateAttributeName(attribute.key, event.target.value)} placeholder="Ví dụ: Màu sắc" />
                                                </div>
                                                <div className="text-xs text-muted-foreground">Nhập các giá trị theo bảng bên dưới.</div>
                                                <div className="justify-self-start md:justify-self-end">
                                                    <Button type="button" variant="outline" size="sm" onClick={() => removeAttribute(attribute.key)}>
                                                ...
                                                <table className="min-w-full text-sm">
                                                    <thead className="bg-muted/40 text-xs uppercase text-muted-foreground">
                                                        <tr>
                                                            ...
                                                            <th className="px-3 py-2 text-left font-medium">Ảnh theo giá trị</th>
                                                        ...
                                                        {attribute.values.map((item) => (
                                                            <tr key={item.key} className="border-t">
                                                                <td className="px-3 py-2 align-top">
                                                                    <Input value={item.value} onChange={(event) => updateAttributeValue(attribute.key, item.key, "value", event.target.value)} placeholder="Đen" />
                                                                </td>
                                                                <td className="px-3 py-2 align-top">
                                                                    <Input value={item.image} onChange={(event) => updateAttributeValue(attribute.key, item.key, "image", event.target.value)} placeholder="https://.../black.jpg" />
                                                                </td>
                                                                <td className="px-3 py-2 text-right align-top">
                                                                    <Button type="button" variant="outline" size="icon" onClick={() => removeAttributeValue(attribute.key, item.key)}>
                                                                        <Trash2 className="h-4 w-4" />
                                                                    </Button>
                                                                </td>
                                                            </tr>
                                            ...
                                            <div className="flex justify-end">
                                                <Button type="button" variant="outline" size="sm" onClick={() => addAttributeValue(attribute.key)}>
                                                    <Plus className="mr-2 h-4 w-4" />
                                                    Thêm giá trị
                                                </Button>
                                            </div>
                        ...
                        <div className="sticky bottom-0 z-30 -mx-5 border-t bg-background/95 px-5 py-3 backdrop-blur">
                            <div className="mx-auto flex max-w-360 flex-wrap items-center justify-between gap-3">
                                <p className="text-xs text-muted-foreground">{isUploadingMedia ? "Đang upload media, vui lòng đợi trước khi lưu." : "Đã tối ưu dạng bảng để nhập nhanh và dễ nhìn."}</p>
                                <div className="flex flex-wrap gap-2">
