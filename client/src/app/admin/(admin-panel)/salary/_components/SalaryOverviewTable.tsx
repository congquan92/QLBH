            ...
            <CardHeader>
                <div className="flex flex-wrap items-start justify-between gap-2">
                    <div>
                        ...
                        <CardDescription className="mt-1">{isLoading ? "Đang tính..." : `${totalOk} nhân viên có lương · ${totalError} không có dữ liệu · Tổng quỹ lương: ${Helper.formatPrice(String(grandTotal))}`}</CardDescription>
                    </div>
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                pagedData.map((item, idx) => {
                                    ...
                                    return (
                                        <tr key={item.user_id} className={`border-b transition-colors ${item.status === "error" ? "bg-red-50/50 hover:bg-red-50" : isTop ? "bg-amber-50/60 hover:bg-amber-50" : "hover:bg-muted/40"}`}>
                                            ...
                                            <td className="px-4 py-3 text-right font-mono">{item.status === "ok" ? Helper.formatPrice(String(item.base_salary)) : "-"}</td>
                                            <td className="px-4 py-3 text-right font-mono text-green-600">
                                                {item.status === "ok" && item.total_holiday_bonus > 0 ? `+${Helper.formatPrice(String(item.total_holiday_bonus))}` : item.status === "ok" ? "0" : "-"}
                                            </td>
                                            <td className="px-4 py-3 text-right font-mono text-amber-600">
                                                {item.status === "ok" && (item.total_manual_bonus ?? 0) > 0 ? `+${Helper.formatPrice(String(item.total_manual_bonus))}` : item.status === "ok" ? "0" : "-"}
                                            </td>
                                            <td className="px-4 py-3 text-right font-mono font-bold">{item.status === "ok" ? Helper.formatPrice(String(item.final_salary)) : "-"}</td>
                                            <td className="px-4 py-3 text-center">
                                                {item.status === "ok" ? (
                                                    <Badge className="bg-green-100 text-green-700 hover:bg-green-100">Thành công</Badge>
                                                ) : (
                                                    <Badge className="bg-red-100 text-red-700 hover:bg-red-100 cursor-help" title={item.error ?? "Lỗi không xác định"}>
                                                        Thiếu dữ liệu
                                                    </Badge>
                                                )}
                                            </td>
                        ...
                        {!isLoading && data.length > 0 && (
                            <tfoot>
                                <tr className="border-t-2 bg-muted/30 font-semibold">
                                    ...
                                    <td className="px-4 py-3 text-right font-mono font-bold text-primary">{Helper.formatPrice(String(grandTotal))}</td>
                                    <td />
                                </tr>
                            </tfoot>
                        )}
