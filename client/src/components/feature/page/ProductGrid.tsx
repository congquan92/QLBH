    ...
    return (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 md:gap-6">
            {products.map((product) => {
                ...
                return (
                    <article key={product.id} className="group overflow-hidden border border-gray-200 bg-white transition-all duration-300 hover:border-red-500 hover:shadow-lg">
                        <Link href={href} className="relative block aspect-square overflow-hidden bg-gray-100">
                            <Image src={product.urlImageCover} alt={product.name} fill className="object-cover transition-transform duration-300 group-hover:scale-105" sizes="(max-width: 768px) 50vw, (max-width: 1200px) 25vw, 20vw" />
                        ...
                        <div className="space-y-3 p-3 md:p-4">
                            ...
                            <div className="grid grid-cols-2 gap-2">
                                <Link href={href} className="inline-flex items-center justify-center gap-2 border border-gray-300 px-3 py-2 text-sm font-medium text-gray-700 transition-colors hover:border-gray-900 hover:text-gray-900">
                                    <Eye className="size-4" />
                                    Chi tiết
                                </Link>
                                <Link href={href} className="inline-flex items-center justify-center gap-2 bg-red-600 px-3 py-2 text-sm font-medium text-white transition-colors hover:bg-red-700">
